...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO users (name, username, email, password, phone, whatsapp, address, user_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (username, username, email, hashed_password, phone, whatsapp, address, user_type))
            conn.commit()
            conn.close()
...
def product_details(product_id):
    ...
    # Fetch product details, including size_chart
    cursor.execute("""
    SELECT p.id, p.name, p.description, p.price, p.image, p.category, 
           p.stock_limit, p.size_chart, 
...
def update_cart_quantity(product_id):
    ...
    if product:
        ...
        if stock_limit is not None and new_quantity > stock_limit:
            flash(f"Error: Quantity for '{product_name}' exceeds available stock (Max: {stock_limit}).", "error")
            conn.close()
            return redirect(url_for("cart"))
